...
| **Prune** | `refresh` removing dangling refs from a manifest. Only ever removes what does not resolve — never rewrites or reorders a working mapping — and only under `--prune` or an interactive confirmation. Since ADR-0004 the `--prune` **flag** also gates repairing a renamed ref; prune itself still means removal alone. |
