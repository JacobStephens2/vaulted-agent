...
pub const PASSTHROUGH: &[&str] = &[
    "HOME", "PATH", "USER", "LOGNAME", "SHELL", "TERM", "COLORTERM", "TZ", "LANG", "LC_ALL",
];
...
mod tests {
    ...
    fn parent_secret_not_in_child_env() {
        ...
        let secrets = HashMap::from([(
            "APP_DB_PASS".to_string(),
            SecretValue::new("injected-only"),
        )]);
        let child = build_child_env(&[], &secrets);
        assert!(!child.contains_key(OsStr::new("PARENT_ONLY_SECRET")));
        assert_eq!(
            child.get(OsStr::new("APP_DB_PASS")).map(|s| s.to_string_lossy()),
