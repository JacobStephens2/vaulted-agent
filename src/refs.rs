...
/// How many mappings `write_refs_merge` added, and how many it recovered from
/// a 0.3.0 glued line.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct RefsMerge {
    pub added: usize,
    pub recovered: usize,
}
...
pub fn split_glued_bitwarden_text(text: &str) -> (String, usize) {
...
pub fn write_refs_merge(
    ...
    let (existing, recovered) = split_glued_bitwarden_text(&existing);
...
mod tests {
    ...
    fn merge_does_not_stack_a_banner_on_every_run() {
        ...
        assert_eq!(write_refs_merge(&p, &first, None, "test").unwrap().added, 1);
        let second = vec![(
            "id2".to_string(),
        ...
        assert_eq!(
            write_refs_merge(&p, &second, None, "test").unwrap().added,
            1
        );
    ...
    fn merge_opens_a_new_banner_below_an_operator_header() {
        ...
        assert_eq!(
            write_refs_merge(&p, &secrets, None, "test").unwrap().added,
            1
        );
        let body = fs::read_to_string(&p).unwrap();
        assert_eq!(
    ...
    fn merge_sees_a_secret_still_mapped_under_its_old_key() {
        ...
        assert_eq!(added.added, 0, "{}", fs::read_to_string(&p).unwrap());
    }
    ...
    fn merge_splits_a_glued_line_even_when_every_secret_looks_already_mapped() {
        ...
        .unwrap();
        let secrets = vec![
            ("id-o".into(), "OPENAI_API_KEY".into(), String::new()),
            ("id-m".into(), "META_AI_API_KEY".into(), String::new()),
            ("id-f".into(), "FIREWORKS_API_KEY".into(), String::new()),
        ];
        let out = write_refs_merge(&p, &secrets, None, "test").unwrap();
